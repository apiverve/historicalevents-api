...
class HistoricaleventsValidationException implements Exception {
  final List<String> errors;

  HistoricaleventsValidationException(this.errors);

  @override
  String toString() => 'HistoricaleventsValidationException: ${errors.join("; ")}';
}
...
/// Historical Events API client.
///
/// For more information, visit: https://apiverve.com/marketplace/historicalevents?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [text] (required) - The keyword for which you want to get the historical events (e.g., moon landing)
class HistoricaleventsClient {
  final String apiKey;
  ...
  HistoricaleventsClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/historicalevents',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [HistoricaleventsValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw HistoricaleventsValidationException(errors);
    }
  }

  /// Execute a request to the Historical Events API.
  ///
  /// Parameters are validated before sending the request.
  Future<HistoricaleventsResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw HistoricaleventsException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return HistoricaleventsResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw HistoricaleventsException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw HistoricaleventsException('Resource not found');
      } else {
        throw HistoricaleventsException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is HistoricaleventsException) rethrow;
      throw HistoricaleventsException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Historical Events API client.
class HistoricaleventsException implements Exception {
  final String message;

  HistoricaleventsException(this.message);

  @override
  String toString() => 'HistoricaleventsException: $message';
}
